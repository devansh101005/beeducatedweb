    ...
    <div className="min-h-screen bg-white dark:bg-slate-900">
            ...
            {[
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <h4 className="font-heading text-base font-bold text-[#0a1e3d] dark:text-slate-50 mb-2">{title}</h4>
                  <p className="font-body text-[13px] text-gray-500 dark:text-slate-400 leading-relaxed">{line1}</p>
                  <p className="font-body text-[13px] text-gray-500 dark:text-slate-400 leading-relaxed">{line2}</p>
                  {link && (
                    <a
                      href={link.href}
      ...
      <section className="py-16 bg-gray-50 dark:bg-slate-900">
        <div className="max-w-6xl mx-auto px-5">
          <div className="grid lg:grid-cols-2 gap-10">
            ...
            {/* Left: Contact Form */}
            <div className="group relative bg-white dark:bg-slate-800 rounded-2xl p-8 sm:p-10 border border-gray-100 dark:border-slate-700 overflow-hidden shadow-soft-md">
              {/* Decorative elements */}
              ...
              <div className="relative z-10">
                ...
                <h3 className="font-heading text-2xl font-bold text-[#0a1e3d] dark:text-slate-50 mb-1">Send us a Message</h3>
                <p className="font-body text-sm text-gray-400 dark:text-slate-400 mb-8">We'll get back to you within 24 hours.</p>

                {error && (
                  <div className="bg-rose-50 dark:bg-rose-900/30 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 px-4 py-3 rounded-xl mb-6 font-body text-sm">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label htmlFor="firstName" className="block font-body text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">First Name</label>
                      <input
                        type="text" id="firstName" name="firstName"
                        value={formData.firstName} onChange={handleChange} required
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-600 font-body text-sm text-[#0a1e3d] dark:text-slate-200 bg-gray-50 dark:bg-slate-800 outline-none focus:border-[#05308d] focus:ring-2 focus:ring-[#05308d]/10 focus:bg-white dark:focus:bg-slate-700 transition-all duration-300"
                        placeholder="Enter first name"
                      />
                    </div>
                    <div>
                      <label htmlFor="lastName" className="block font-body text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Last Name</label>
                      <input
                  ...
                  <div>
                    <label htmlFor="email" className="block font-body text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Email Address</label>
                    <input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label htmlFor="role" className="block font-body text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">I am a</label>
                      <select
                        id="role" name="role"
                        value={formData.role} onChange={handleChange} required
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-600 font-body text-sm text-[#0a1e3d] dark:text-slate-200 bg-gray-50 dark:bg-slate-800 outline-none focus:border-[#05308d] focus:ring-2 focus:ring-[#05308d]/10 focus:bg-white dark:focus:bg-slate-700 transition-all duration-300 appearance-none cursor-pointer"
                      >
                    ...
                    <div>
                      <label htmlFor="department" className="block font-body text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Inquiry For</label>
                      <select
                  ...
                  <div>
                    <label htmlFor="message" className="block font-body text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Message</label>
                    <textarea
                      id="message" name="message"
                      value={formData.message} onChange={handleChange} required rows="4"
                      minLength={10}
                      maxLength={MESSAGE_LIMIT}
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-600 font-body text-sm text-[#0a1e3d] dark:text-slate-200 bg-gray-50 dark:bg-slate-800 outline-none focus:border-[#05308d] focus:ring-2 focus:ring-[#05308d]/10 focus:bg-white dark:focus:bg-slate-700 transition-all duration-300 resize-none"
                    ...
                    <div className="flex justify-between mt-1">
                      ...
                      <span className={`font-body text-xs ${formData.message.length > MESSAGE_LIMIT * 0.9 ? 'text-amber-500' : 'text-gray-400 dark:text-slate-500'} ${formData.message.length >= MESSAGE_LIMIT ? '!text-red-500 font-semibold' : ''}`}>
            ...
            <div className="flex flex-col gap-6">
              {/* Map */}
              <div className="relative rounded-2xl overflow-hidden border border-gray-100 dark:border-slate-700 shadow-soft-md flex-1 min-h-[300px]">
                <iframe
      ...
      {success && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-5">
          ...
          {/* Modal */}
          <div className="relative bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden animate-[popIn_0.4s_ease]">
            {/* Close button */}
            <button
              onClick={() => setSuccess(false)}
              className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-black/5 dark:bg-white/10 flex items-center justify-center text-gray-400 dark:text-slate-500 hover:bg-black/10 dark:hover:bg-white/20 hover:text-gray-600 dark:hover:text-slate-300 transition-all cursor-pointer border-none"
            >
            ...
            <div className="px-8 py-7 text-center">
              <p className="font-body text-sm text-gray-500 dark:text-slate-400 leading-relaxed mb-6">
                Thank you for reaching out to us. Our team will review your message and get back to you
                <span className="font-semibold text-[#0a1e3d] dark:text-slate-50"> within 24 hours</span>.
              </p>
